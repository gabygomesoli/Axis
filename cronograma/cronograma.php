...
document.getElementById('prev-week')?.addEventListener('click', function() {
    const currentWeek = <?php echo $semanaAtual; ?>;
    const prevWeek = currentWeek - 1;
    if (prevWeek >= <?php echo min($semanasDisponiveis); ?>) {
        window.location.href = `?semana=${prevWeek}`;
    }
});
...
document.getElementById('next-week')?.addEventListener('click', function() {
    ...
    if (nextWeek <= <?php echo max($semanasDisponiveis); ?>) {
        window.location.href = `?semana=${nextWeek}`;
    }
});
</script>